...
def run_validation(dates):
    ...
    first = dates[0]
    print(first)
    ok_cmems_date = is_master_consecutive(master_path_cmems_pdf, first)
    print(ok_cmems_date)
    ok_neurost_date = is_master_consecutive(master_path_neurost_pdf, first)
    print(ok_neurost_date)

    if ok_cmems_date and ok_neurost_date == False:
    ...
    for (year, month), dates in by_month.items():
        ...
        if ok_neurost_date:
            ...
            neurost_files = glob.glob(master_path_neurost)
            print(neurost_files)
            ds_master_neurost = xr.open_dataset(neurost_files[0])
            ...
            append_to_master(year, month, df_neurost_start_date, df_neurost_end_date, 'neurost', validation_mode_neurost)
